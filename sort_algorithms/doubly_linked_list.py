...
class DoubleLinkedList(Generic[T]):
    ...
    def sort_course(self) -> None:
        ...
        for i in range(1, self._length):
            ...
            it = i
            while it > 0 and self.__getitem__(it - 1).key2() < temp.key2():
                node = self.get_pointer(self.__getitem__(it))
                node.data = node.prev_ptr.data
                it -= 1
            node = self.get_pointer(self.__getitem__(it))
            node.data = temp

    def sort_engine_capacity(self) -> None:
        if self._length == 0:
            raise ValueError("array is empty")
        ...
        for i in range(0, self._length):
            ...
            for j in range(i, self._length):
                if not self.__getitem__(j).key() > self.__getitem__(minimum).key():
                    minimum = j
            
            node = self.get_pointer(self.__getitem__(i))
            memory_node = node.data
            node_min = self.get_pointer(self.__getitem__(minimum))
            node.data = node_min.data
            node_min.data = memory_node

    def sort_average_speed(self) -> None:
        if self._length == 0:
            raise ValueError("array is empty")
        ...
        while step >= 1:
            i = 0
            while i + step < self._length:
                if self.__getitem__(i).key2() < self.__getitem__(i + int(step)).key2():
                    node = self.get_pointer(self.__getitem__(i))
                    memory_node = node.data
                    node_min = self.get_pointer(self.__getitem__(i + int(step)))
                    node.data = node_min.data
                    node_min.data = memory_node
                i += 1
            step /= factor

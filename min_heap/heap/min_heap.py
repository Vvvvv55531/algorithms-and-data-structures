...
class Student(IKey):
    ...
    def __str__(self) -> str:
        return f"{self.fio}, {self.group_number}, {self.course}, {self.age}, {self.average_mark}"
...
class Heap(Generic[T]):
    ...
    def create_heap_from_list(data: list[T],
        ...
        struct: Heap[T] = Heap[T](size=len(data), fixed=fixed, comp=comp)
        for it in data:
            struct.insert(it)
        return struct
    ...
    def load() -> list[Student]:
        storage: list[Student] = []
        with (open("heap_save.txt", "r") as file):
            for line in file:
                string: str = line.strip()
                elements: list[str] = string.split(", ")  # Разделяем строку на части
                if len(elements) == 5:
                    fio, group_number, course, age, average_mark = elements
                    student = Student(str(fio), int(group_number), int(course), int(age), float(average_mark))
